...
export function registerRoutes(app) {
    ...
    app.post("/api/media/upload", authMiddleware, (req, res, next) => {
        ...
        const file = req.file; // multer-s3 adds key and location
        if (!file) {
    ...
    // ── Products & Catalog ────────────────────────────────────────────────────
    app.post("/api/catalog/import", authMiddleware, xmlUpload.single("file"), async (req, res) => {
        const file = req.file;
        const { url } = req.body;
        if (!file && !url) {
    ...
    app.get("/api/catalog/imports", authMiddleware, async (_req, res) => {
    ...
    app.post("/api/catalog/sync", authMiddleware, async (req, res) => {
        try {
    ...
    app.get("/api/catalog/syncs", authMiddleware, async (req, res) => {
        const list = await db.select().from(catalogSyncs).orderBy(desc(catalogSyncs.createdAt));
        res.json({ syncs: list });
    });
    app.delete("/api/catalog/syncs/:id", authMiddleware, async (req, res) => {
    ...
    app.get("/api/public/carousels/:id", async (req, res) => {
    ...
    app.get("/embed/carousel.js", (_req, res) => {
        res.setHeader("Access-Control-Allow-Origin", "*");
    ...
    app.post("/api/carousels", authMiddleware, async (req, res) => {
        try {
    ...
    app.put("/api/carousels/:id", authMiddleware, async (req, res) => {
        const carouselId = parseInt(req.params.id);
        ...
        try {
            ...
            if (videoIds && Array.isArray(videoIds)) {
                ...
                if (videoIds.length > 0) {
                    await db.insert(carouselVideos).values(videoIds.map((vid, idx) => ({
                        carouselId,
                        videoId: vid,
                        position: idx,
                    })));
    ...
    app.delete("/api/carousels/:id", authMiddleware, async (req, res) => {
    ...
    app.get("/api/videos", authMiddleware, async (_req, res) => {
    ...
    app.get("/api/products", authMiddleware, async (req, res) => {
        ...
        if (search) {
            condition = or(ilike(products.title, `%${search}%`), ilike(products.externalId, `%${search}%`));
        }
